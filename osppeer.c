...
#define MAXFILESIZ 	1000000 // Max size we are downloading, this is set to 1MB since the cat pictures are max 40kb
				// Tracker should keep track of the file sizes we want to download. In this case, we'll
				// just use 1mb
#define MAXNUMUPLOADSALLOWED 	8	//Maximum number of uploads allowed (Infinite fork protection)
...
task_t *start_download(task_t *tracker_task, const char *filename)
{
	...
	size_t filename_len;
	filename_len = strnlen(filename,FILENAMESIZ);
	strncpy(t->filename, filename,filename_len);
	t->filename[filename_len] = '\0'; //makes sure the c-string ends with a null byte
...
static void task_download(task_t *t, task_t *tracker_task)
{
	...
	if(evil_mode == 0) //Not evil, just ask for the file with filename
		osp2p_writef(t->peer_fd, "GET %s OSP2P\n", t->filename); //This writes the tracker/peer fd
	else { //Evil mode active, request from the peer a file with a name of 496 bytes.
		int i;
    char evilName[5000];
    for(i=0; i < 5000; i++)
             evilName[i] = 'a';
    //char evilName[5000] = "../answers.txt"; //If we want to access a dif directory
		osp2p_writef(t->peer_fd, "GET %s OSP2P\n", evilName);
	}
	...
	for (i = 0; i < 50; i++) {
		...
		{
			size_t filename_len;
			filename_len= strnlen(t->filename,FILENAMESIZ);
			strncpy(t->disk_filename, t->filename, filename_len);
		}
		else
...
* Try 'rm %s.~*~' to remove them.\n", t->filename, t->filename);
	...
	// and write it from the task buffer onto disk. //Important
	while (1) {
		int ret = read_to_taskbuf(t->peer_fd, t);
		if (ret == TBUF_ERROR) {
		...
		if (t->total_written > MAXFILESIZ) {
			message("* Download file is larger than MAXFILESIZ allowed of: '%d' bytes and will be removed\n",
					 MAXFILESIZ);
	...
	if (t->total_written > 0) {
		...
		{			
			...
			char *buffer;
			char *decrypted;
			buffer = calloc(1,t->total_written);
			decrypted = calloc(1,t->total_written);
			read(t->disk_fd,buffer,t->total_written);
			memcpy(decrypted,Decrypt(key,buffer,t->total_written), t->total_written);
			close(t->disk_fd);
			remove(t->disk_filename);
			t->disk_fd = open(t->disk_filename,
					  O_WRONLY | O_CREAT | O_EXCL, 0666);
			write(t->disk_fd,decrypted,t->total_written);
		}
...
static void task_upload(task_t *t)
{
	...
	if(t->tail > FILENAMESIZ) {
		error("* Peer tried to request a file whose name length exceeds buffer size! (Request Declined!)");
		goto exit;
	}
	...
	int i;
	int filename_len = strlen(t->filename);
	for(i=0;i<filename_len;i++)
	{
		if(t->filename[i]=='/' || t->filename[0] == '\0')
		{
			error("* Peer attempted to access file not in current directory %s (will now exit)", t->filename);
			goto exit;
		}
	}
	...
	if(evil_mode)
	{
		...
		message("* Transferring never ending file...\n");
	...
	else
	{
		if(encryption_mode)
		{
			...
			//Open file
			file = fopen(t->filename, "rb");
			if (!file)
			{
				fprintf(stderr, "Unable to open file %s", t->filename);
				return;
			}
			...
			encrypted=(char *)malloc(fileLen+1);
			if (!buffer)
			{
			...
			memcpy(encrypted,Encrypt(key,buffer,fileLen+1), fileLen+1);
			write(tempFd,encrypted,fileLen+1);
			t->disk_fd = tempFd;
...
"         -b[MODE]     Evil mode!!!!!!!!\n");
	...
	register_files(tracker_task, myalias);
	int noOfDownloads=0, numUploads=0;
	int i;
	int status;
	...
	{
		if(numUploads > MAXNUMUPLOADSALLOWED) { //Infinite fork prevention, if we've reached max allowed
							//number of uploading processes, wait until 1 finishes.
			waitpid(-1, &status, 0);
			numUploads--;
		}
		else
